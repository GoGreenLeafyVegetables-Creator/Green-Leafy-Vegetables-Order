...
const CustomerBill: React.FC<CustomerBillProps> = ({
  ...
  const generateUPIQRCode = (amount: number) => {
    ...
    const upiString = `upi://pay?pa=${upiId}&pn=SHREE%20GANESHA%20GREEN%20LEAFY%20VEGETABLES&am=${amount}&cu=INR&tn=Order%20${order.id}`;
    return `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(upiString)}`;
  };

  const generateCustomerPageQR = () => {
    const customerUrl = `${window.location.origin}/customer/${customer.qr_code}`;
    return `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(customerUrl)}`;
  };
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .qr-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 20px 0; }
            .qr-box { text-align: center; padding: 15px; border: 1px solid #ddd; border-radius: 8px; }
