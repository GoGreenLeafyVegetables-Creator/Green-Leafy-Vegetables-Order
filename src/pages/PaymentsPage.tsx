    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {filteredPayments.map((payment) => (
          <Card key={payment.order.id}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <h3 className="font-semibold">{payment.customer_name}</h3>
                    {getStatusBadge(payment.order.payment_status, payment.order.balance_amount)}
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <div className="flex items-center gap-1">
                      ...
                      {format(new Date(payment.order.order_date), "dd/MM/yyyy")}
                    </div>
                    <div className="flex items-center gap-1">
                      <IndianRupee className="h-3 w-3" />
                      Total: ₹{payment.order.total_amount.toFixed(2)}
                    </div>
                  </div>
                </div>
                ...
                <div className="text-right space-y-1">
                  <div className="text-lg font-semibold text-green-600">
                    ₹{payment.order.paid_amount.toFixed(2)} Paid
                  </div>
                  {payment.order.balance_amount > 0 && (
                    <div className="text-sm text-red-600">
                      ₹{payment.order.balance_amount.toFixed(2)} Due
                    </div>
                  )}
                  ...
                  <div className="flex gap-2 mt-2">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleEditPayment(payment.order)}
                    >
